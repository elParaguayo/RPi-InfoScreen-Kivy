...
class HelsinkiBusStop(Screen):
    ...
    def __init__(self, **kwargs):
        super(HelsinkiBusStop, self).__init__(**kwargs)
    ...
    def draw_buses(self):
        ...
        # Loop over the buses, create a HelsinkiBus object and add it to the
        # StackLayout
        for bus in buses:
            bs = HelsinkiBus(bus=bus)
            sl.add_widget(bs)
...
class HelsinkiBusScreen(Screen):
    """Base screen object for Helsinki Buses.

    Has a screenmanager to hold screens for specific bus stops.
    """
    def __init__(self, **kwargs):
        super(HelsinkiBusScreen, self).__init__(**kwargs)
    ...
    def on_enter(self):
        ...
        if not self.running:
            for stop in self.stops:
                nm = str(stop["stopid"])
                self.scrmgr.add_widget(HelsinkiBusStop(stop=stop, name=nm))
            self.running = True
